...
i = 0
with open(ResourcesFolder + 'training.csv', 'r') as train_stream:
    for line in train_stream:
        current_line = np.array(list(map(int, line.split(','))), dtype=np.int16)
        labels[i] = current_line[-1]
        sparse_training[i,:-1] = current_line[1:-1] / np.sum(current_line[1:-1])
        i += 1
        print(i)
sparse.save_npz(ResourcesFolder + 'lr_training_data_norm', sparse_training.tocsr())
